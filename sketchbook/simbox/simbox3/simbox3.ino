  ...
  if (client) {
    ...
    while (client.connected()) {
      if (client.available()) {
          ...
          char c =client.read();     // writing data into array
          bufferWEB[countWEB++] = c;
        client.print("Received char ->");
        client.println(c);
        //char c = client.read();
        // if you've gotten to the end of the line (received a newline
        // character) and the line is blank, the http request has ended,
        // so you can send a reply
        if (strcmp (bufferWEB, "404#\r") == 0 && currentLineIsBlank)  {
        //if (bufferWEB[countWEB] == '\n' && currentLineIsBlank) {
          ...
          Serial.println("****** END WEBSERVER ******");
          clearBufferArray3();
          countWEB=0;
          break;
        }
        client.print("Bufferè ->");
        client.println(bufferWEB);
        if (c == '\r') {
         
           client.println("carriage return detected!");
          
        }
        client.println((int)c);
        if (c == '\n') {
